...
<?php $_postrow_count = (isset($this->_tpldata['postrow'])) ? sizeof($this->_tpldata['postrow']) : 0;if ($_postrow_count) {for ($_postrow_i = 0; $_postrow_i < $_postrow_count; ++$_postrow_i){$_postrow_val = &$this->_tpldata['postrow'][$_postrow_i]; ?>
	...
	<?php } else { ?>
			...
			<td align="center" valign="middle">
				...
				<b class="postauthor"<?php if ($_postrow_val['POST_AUTHOR_COLOUR']) {  ?> style="color: <?php echo $_postrow_val['POST_AUTHOR_COLOUR']; ?>"<?php } ?>><?php if ($this->_rootref['QUICK_REPLY']) {  ?><a href="#postform" onclick="insert_text('[b]<?php echo $_postrow_val['POSTER_QUOTE']; ?>[/b]\r\n'); return false;"<?php if ($_postrow_val['POST_AUTHOR_COLOUR']) {  ?> style="color: <?php echo $_postrow_val['POST_AUTHOR_COLOUR']; ?>"<?php } ?>><?php } echo $_postrow_val['POST_AUTHOR']; if ($this->_rootref['QUICK_REPLY']) {  ?></a><?php } ?></b>
			</td>
		...
		<?php if (!($_postrow_val['S_ROW_COUNT'] & 1)  ) {  ?><tr class="row1"><?php } else { ?><tr class="row2"><?php } ?>
			...
			<?php } if ($this->_rootref['QUICK_REPLY']) {  ?>
				...
				<tr>
					<td class="postdetails"><a href="#postform" onclick="addquote(<?php echo $_postrow_val['POST_ID']; ?>,'<?php echo $_postrow_val['POSTER_QUOTE']; ?>'); insert_text('\r\n'); return false;" title="<?php echo ((isset($this->_rootref['L_QUOTE_TEXT'])) ? $this->_rootref['L_QUOTE_TEXT'] : ((isset($user->lang['QUOTE_TEXT'])) ? $user->lang['QUOTE_TEXT'] : '{ QUOTE_TEXT }')); ?>"><?php echo ((isset($this->_rootref['L_QUOTE'])) ? $this->_rootref['L_QUOTE'] : ((isset($user->lang['QUOTE'])) ? $user->lang['QUOTE'] : '{ QUOTE }')); ?></a></td>
				</tr>
			...
			<?php } ?>
				...
				<span class="postdetails">
					<?php if ($_postrow_val['POSTER_JOINED']) {  ?><br /><b><?php echo ((isset($this->_rootref['L_JOINED'])) ? $this->_rootref['L_JOINED'] : ((isset($user->lang['JOINED'])) ? $user->lang['JOINED'] : '{ JOINED }')); ?>:</b> <?php echo $_postrow_val['POSTER_JOINED']; } if ($_postrow_val['POSTER_POSTS'] != ('')) {  ?><br /><b><?php echo ((isset($this->_rootref['L_POSTS'])) ? $this->_rootref['L_POSTS'] : ((isset($user->lang['POSTS'])) ? $user->lang['POSTS'] : '{ POSTS }')); ?>:</b> <?php echo $_postrow_val['POSTER_POSTS']; } if ($_postrow_val['POSTER_FROM']) {  ?><br /><b><?php echo ((isset($this->_rootref['L_LOCATION'])) ? $this->_rootref['L_LOCATION'] : ((isset($user->lang['LOCATION'])) ? $user->lang['LOCATION'] : '{ LOCATION }')); ?>:</b> <?php echo $_postrow_val['POSTER_FROM']; } if (! $_postrow_val['S_POST_ANONYMOUS'] && $_postrow_val['S_FORUM_THANKS']) {  ?><br /><b><?php echo ((isset($this->_rootref['L_GIVEN'])) ? $this->_rootref['L_GIVEN'] : ((isset($user->lang['GIVEN'])) ? $user->lang['GIVEN'] : '{ GIVEN }')); ?>:</b> <?php echo $_postrow_val['POSTER_GIVE_COUNT']; ?> <?php echo ((isset($this->_rootref['L_THANKS'])) ? $this->_rootref['L_THANKS'] : ((isset($user->lang['THANKS'])) ? $user->lang['THANKS'] : '{ THANKS }')); ?><br /><b><?php echo ((isset($this->_rootref['L_RECEIVED'])) ? $this->_rootref['L_RECEIVED'] : ((isset($user->lang['RECEIVED'])) ? $user->lang['RECEIVED'] : '{ RECEIVED }')); ?>:</b> <?php echo $_postrow_val['POSTER_RECEIVE_COUNT']; ?> <?php echo ((isset($this->_rootref['L_THANKS'])) ? $this->_rootref['L_THANKS'] : ((isset($user->lang['THANKS'])) ? $user->lang['THANKS'] : '{ THANKS }')); } if ($_postrow_val['S_PROFILE_FIELD1']) {  ?><!-- Use a construct like this to include admin defined profile fields. Replace FIELD1 with the name of your field. -->
				...
				<tr>
					...
					<?php } if (! $_postrow_val['S_HAS_ATTACHMENTS']) {  ?><br clear="all" /><br /><?php } ?>
						...
						<tr valign="middle">
							...
							<?php if (! $this->_rootref['S_IS_BOT']) {  if ($_postrow_val['U_REPORT']) {  ?><a href="<?php echo $_postrow_val['U_REPORT']; ?>"><?php echo (isset($this->_rootref['REPORT_IMG'])) ? $this->_rootref['REPORT_IMG'] : ''; ?></a> <?php } if ($_postrow_val['U_INFO']) {  ?><a href="<?php echo $_postrow_val['U_INFO']; ?>"><?php echo (isset($this->_rootref['INFO_IMG'])) ? $this->_rootref['INFO_IMG'] : ''; ?></a> <?php } if ($_postrow_val['U_WARN']) {  ?><a href="<?php echo $_postrow_val['U_WARN']; ?>"><?php echo (isset($this->_rootref['WARN_IMG'])) ? $this->_rootref['WARN_IMG'] : ''; ?></a> <?php } if ($_postrow_val['U_DELETE']) {  ?><a href="<?php echo $_postrow_val['U_DELETE']; ?>"><?php echo (isset($this->_rootref['DELETE_IMG'])) ? $this->_rootref['DELETE_IMG'] : ''; ?></a> <?php } if (! $_postrow_val['S_POST_ANONYMOUS'] && ! $this->_rootref['S_IS_BOT']) {  if ($_postrow_val['S_FORUM_THANKS'] && $this->_rootref['S_USER_LOGGED_IN'] && ! $_postrow_val['S_IS_OWN_POST'] && ( ! $_postrow_val['S_ALREADY_THANKED'] || $_postrow_val['S_REMOVE_THANKS'] )) {  ?>

										<a href="<?php echo $_postrow_val['THANKS_LINK']; ?>" alt="<?php echo $_postrow_val['THANK_ALT']; echo $_postrow_val['POST_AUTHOR']; ?>" title="<?php echo $_postrow_val['THANK_ALT']; echo $_postrow_val['POST_AUTHOR']; ?>"><?php echo $_postrow_val['THANK_IMG']; ?></a>
									<?php } if ($_postrow_val['THANKS'] && $_postrow_val['THANKS_POSTLIST_VIEW']) {  ?>

										<table width="100%" cellspacing="0">
											<tr valign="middle">
												<td class="gensmall" align="<?php echo (isset($this->_rootref['S_CONTENT_FLOW_BEGIN'])) ? $this->_rootref['S_CONTENT_FLOW_BEGIN'] : ''; ?>">
												<hr />
												<?php echo $_postrow_val['THANK_TEXT']; echo $_postrow_val['POST_AUTHOR_FULL']; echo $_postrow_val['THANK_TEXT_2']; ?>: <?php echo $_postrow_val['THANKS']; ?></td>
											</tr>
										</table>
									<?php } } } ?>
